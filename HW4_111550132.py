...
def build_prompt(full_context: str, current_step: str, options: Dict[str, dict]) -> str:
    """Concise / HW3-style：簡短指令、強制只吐字母、零解釋。對齊 HW4_111550132.py。"""
    options_block = "\n".join(
...
def _wrap_qwen3_no_think(user_msg: str) -> str:
    """Qwen3.6 ChatML wrap + 預填空 `<think>\n\n</think>` 跳過 thinking 階段。

    觀察：Qwen3.6 是 thinking 模型，*必須* 預填空 `<think>...</think>` 區塊
    才能避免重新進入 thinking；若改成直接 `Answer: ` 開頭，模型反而會輸出
    `<think>` token (約 40-100% 樣本失敗)。維持空 think block、不再加
    `Answer: ` 預填；搭配 concise build_prompt 模型會吐極短 essay + `Answer: X`，
    交給 multi-tier parser 抽 letter。
    """
    return (
        "<|im_start|>user\n"
        + user_msg
