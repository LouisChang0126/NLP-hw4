...
# Decoding 參數，main() 會用 CLI 旗標覆寫這兩個 global
DECODE_TEMPERATURE: float = 0.0
DECODE_TOP_P: float = 0.1
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--temperature", type=float, default=DECODE_TEMPERATURE,
                        help=f"sampling temperature (預設 {DECODE_TEMPERATURE})")
    parser.add_argument("--top-p", type=float, default=DECODE_TOP_P,
                        help=f"sampling top-p (預設 {DECODE_TOP_P})")
...
def main() -> None:
    ...
    global DECODE_TEMPERATURE, DECODE_TOP_P
